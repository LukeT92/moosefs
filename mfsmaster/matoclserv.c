...
static inline int matoclserv_fuse_write_chunk_common(matoclserventry *eptr,uint32_t msgid,uint32_t inode,uint32_t indx,uint8_t chunkopflags) {
	...
	if (sessions_get_sesflags(eptr->sesdata)&SESFLAG_READONLY) {
		...
		status = fs_writechunk(inode,indx,chunkopflags,&prevchunkid,&chunkid,&fleng,eptr->peerip,&opflag);
	}
...
//		locked = 0;
	...
	ptr = matoclserv_createpacket(eptr,MATOCL_FUSE_TRUNCATE,(status!=MFS_STATUS_OK)?5:(eptr->asize+4));
	put32bit(&ptr,msgid);
	if (status!=MFS_STATUS_OK) {
		put8bit(&ptr,status);
	} else {
...
void matoclserv_fuse_create(matoclserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (status==MFS_STATUS_OK) {
		if (CreateFirstChunk) {
			...
			swchunks *swc;
			/* create first chunk */
			if (fs_writechunk(newinode,0,0,&prevchunkid,&chunkid,&fleng,eptr->peerip,&opflag)==MFS_STATUS_OK) {
