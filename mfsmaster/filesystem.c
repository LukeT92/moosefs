...
uint8_t fs_writechunk(uint32_t inode,uint32_t indx,uint8_t chunkopflags,uint64_t *prevchunkid,uint64_t *chunkid,uint64_t *length,uint32_t peerip,uint8_t *opflag) {
	int status;
	uint32_t i;
...
//	}
	...
	ochunkid = p->data.fdata.chunktab[indx];
	*prevchunkid = ochunkid;
	status = chunk_multi_modify((chunkopflags&CHUNKOPFLAG_CONTINUEOP)?1:0,&nchunkid,ochunkid,p->sclassid,peerip,opflag);
	if (status!=MFS_STATUS_OK) {
		return status;
	}
	p->data.fdata.chunktab[indx] = nchunkid;
